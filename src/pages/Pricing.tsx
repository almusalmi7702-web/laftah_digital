        ...
        {displayPlans ? (
          ...
          <div className="grid md:grid-cols-3 gap-8 items-start">
            {displayPlans.map((pkg, i) => (
              ...
              >
                <h3 className={`text-xl font-bold mb-2 text-right ${pkg.is_featured ? 'text-white' : 'text-navy-800'}`}>
                  {pkg.name}
                </h3>
                <div className="flex items-baseline justify-end gap-1 mb-8">
                  <span className={`text-sm ${pkg.is_featured ? 'text-teal-100' : 'text-gray-500'}`}>ر.س / شهريًا</span>
                  <span className={`text-5xl font-black ${pkg.is_featured ? 'text-white' : 'text-teal-600'}`}>{pkg.price}</span>
                </div>
                <ul className="space-y-4 mb-8">
                  {pkg.features.map((f, fi) => (
                    <li key={fi} className={`flex items-center gap-3 justify-end text-sm ${pkg.is_featured ? 'text-teal-50' : 'text-gray-600'}`}>
                      {f}
                      <Check className={`w-5 h-5 flex-shrink-0 ${pkg.is_featured ? 'text-teal-200' : 'text-teal-500'}`} />
                    </li>
                  ))}
                </ul>
                {pkg.description && (
                  <p className={`text-sm mb-6 text-right ${pkg.is_featured ? 'text-teal-100' : 'text-gray-500'}`}>
                    {pkg.description}
                  </p>
                )}
                <a
  ...
  return (
    <section className="py-16 bg-gray-50" ref={ref}>
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
        <h2 className={`text-2xl font-black text-navy-800 text-center mb-10 transition-all duration-700 ${isInView ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
          أسئلة شائعة
        </h2>
        <div className="space-y-4">
          {faqs.map((faq, i) => (
            ...
            >
              ...
              >
                <span className="text-sm text-gray-500">{openIndex === i ? '−' : '+'}</span>
