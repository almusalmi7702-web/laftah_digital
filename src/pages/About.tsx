    ...
    <div className="pt-20">
      ...
      <section className="py-20 bg-gradient-to-bl from-theme-muted via-theme-page to-theme-primary-soft">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-right" ref={ref}>
          ...
          <h1 className={`text-4xl md:text-5xl font-black text-theme-text mb-8 leading-tight transition-all duration-700 ${isInView ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
            {about.meaningTitle}
          </h1>
      ...
      <section className="py-16 bg-theme-page">
        <div className="max-w-xl mx-auto px-4 text-center">
          <h2 className="text-2xl font-black text-theme-text mb-4">هل أنت مستعد؟</h2>
          <p className="text-theme-text-secondary mb-8">نسعد بمساعدتك على بناء حضور بصري يعكس جودة مشروعك.</p>
          <Link
            to="/free-audit"
...
const StoryCard = ({ icon, title, text, color }: StoryCardProps) => {
  ...
  return (
    <div
      ref={ref}
      className={`bg-theme-surface rounded-2xl p-10 shadow-theme-card border border-theme-border hover:shadow-theme-elevated transition-all duration-500 text-right ${isInView ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}
    >
      <div className={`w-16 h-16 rounded-2xl bg-gradient-to-br ${color} flex items-center justify-center mb-8`}>
        {icon}
      </div>
      <h3 className="text-xl font-bold text-theme-text mb-5">{title}</h3>
      <p className="text-theme-text-secondary leading-[1.9]">{text}</p>
    </div>
  );
};
